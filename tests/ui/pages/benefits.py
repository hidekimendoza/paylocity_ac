...
class BenefitsPage:
    ...
    MODAL_DELETE_TITLE = (
        By.CSS_SELECTOR, "div[id='deleteModal'] h5[class='modal-title']")
    DEL_MODAL = (
        By.XPATH, "//div[@id='deleteModal']//div[@class='modal-body']")
    DEL_CONFIRMATION_BUTTON = (By.XPATH, "//button[@id='deleteEmployee']")
    DEL_CANCEL_BUTTON = (
        By.XPATH, "//div[@id='deleteModal']//button[@type='button'][normalize-space()='Cancel']")
    ...
    MODAL_TITLE = (By.CSS_SELECTOR,
                   "div[id='employeeModal'] h5[class='modal-title']")
    EMPLOYEE_TABLE = (By.ID, "employeesTable")
    EMPLOYEE_ROW_BY_NAME = (
        By.XPATH, "//table[@id='employeesTable']//tr[td[text()='{}']]")
    ...
    def verify_table_element_added(self, first_name):
        ...
        employee_row_locator = (
            By.XPATH, f".//td[contains(text(), {first_name})]")
        try:
            _ = self.wait.until(
                ec.presence_of_element_located(employee_row_locator))
            print(
                f"Successfully added employee '{first_name}'. Employee row is visible.")
        except TimeoutException:
            raise AssertionError(
                f"Employee '{first_name}' did not appear in the table after saving.")
        try:
            total_cost_text = self.driver.find_element(
                *self.BENEFIT_COST_DISPLAY).text
    ...
    def save_add_form(self, wait_table_appears=True):
        ...
        if wait_table_appears:
            second_row = (By.CSS_SELECTOR,
                          "#employeesTable > tbody > tr:nth-child(2)")
    ...
    def edit_employee(self, id, new_first, new_last, new_deps, validate=False):
        """
    ...
    def validate_edit_employee(self, employee_row, new_first):
        ...
        try:
            ...
            employee_row_locator = (
                By.XPATH, f".//td[contains(text(), {new_first})]")
            self.wait.until(
                ec.presence_of_element_located(employee_row_locator))
        except TimeoutException:
            raise AssertionError(
                "Employee data did not change in the table after editing.")
    ...
    def get_existing_employee_from_table(self, id):
        ...
        employee_row_locator = (
            By.XPATH, self.EMPLOYEE_ROW_BY_NAME[1].format(id))
        try:
            employee_row = self.wait.until(
                ec.presence_of_element_located(employee_row_locator))
        except TimeoutException:
            raise NoSuchElementException(
                f"Employee '{id}' not found in the table.")
        return employee_row

    def delete_employee(self, id, verify=False):
        """
    ...
    def validate_employee_deletion(self, employee_row, id):
        ...
        try:
            self.wait.until(ec.staleness_of(employee_row))
            print(
                f"Successfully deleted employee '{id}'. Row is no longer visible.")
        except TimeoutException:
            raise AssertionError(
                f"Employee '{id}' was not deleted from the table.")
    ...
    def get_column_texts(self, column_index):
        ...
        try:
            rows = self.driver.find_elements(
                By.XPATH, "//table[@id='employeesTable']//tr")
