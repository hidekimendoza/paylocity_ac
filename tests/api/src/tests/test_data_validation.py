...
class TestDataValidation:
    ...
    def test_post_required_data_only(self, clean_env, created_employee):
        post_employee = Employee("DupUserName", "DupFirst", "Duplname", id=created_employee["id"])
        client = APIClient()
        response = client.create_employee(post_employee)
    ...
    def test_post_custom_salary_shall_not_change_salary(self, clean_env):
        ...
        json_response = response.json()
        print(json_response)
        check.equal(Employee.GROSS_PAY_PER_CHECK * Employee.NUM_PAYCHECKS_PER_YEAR, json_response["salary"])
    ...
    def test_post_expired_employee_shall_not_be_available(self):
        client = APIClient()
        expired_employee = Employee("uname", "first", "last", expiration="2015-06-22T04:40:35.641Z")
    ...
    def test_update_update_salary(self, created_employee):
        ...
        updated_emp = Employee("UpdatedUsername", "UpdatedFirstname", "UpdatedLastName", salary=75000.0, id=existing_id)
        client = APIClient()
